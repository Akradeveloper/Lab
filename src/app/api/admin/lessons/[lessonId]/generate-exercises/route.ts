...
Para CODE: { "type": "CODE", "question": "enunciado", "language": "javascript"|"python"|"typescript"|"java", "template": "código inicial para el alumno", "testCases": [ { "input": "entrada por stdin", "expectedOutput": "salida esperada" } ] }. Incluir al menos un test case.
...
Genera exactamente ${count} ejercicios. Responde con JSON: { "exercises": [ ... ] }`;
    ...
    const valid = list.filter(isValidGenerated);
    if (valid.length === 0) {
      return NextResponse.json(
        { error: "La IA no generó ejercicios válidos" },
        { status: 502 }
      );
    }
